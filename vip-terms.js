...
if(vipTermsEditor){
document.querySelectorAll('.rich-toolbar button').forEach(btn=>btn.onclick=()=>{vipTermsEditor.focus();document.execCommand(btn.dataset.cmd,false,null)});
document.querySelector('#vipTermsSaveBtn').onclick=()=>{if(!vipTermsEditor.innerText.trim()){alert('请输入VIP条款与规则');return}localStorage.setItem('vipTermsHtml',vipTermsEditor.innerHTML);alert('VIP条款与规则已保存')};
